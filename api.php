<?php
    ...
    $email = email($nome);
    $keys = array(1 => 'pk_live_jkLcp0xPaPMCv9pjf9DQMKdN');
    $key = array_rand($keys);
    $keyStripe = $keys[$key];

    $proxy = curl_exec($ch);
    $username = 'auto';
    $password = '7BPraDr7JaEJ7hfKET4TwmhFk';
    $session = mt_rand();
...
//pk_live_awU8f7I5z9qGzhA6AEtRMjGJ
    ...
    if(strpos($resultado, 'processing_error')) {
        ...
        echo $live;
    }elseif(strpos($resultado, 'Your card number is incorrect.')){
        $retorno = getStr2($resultado, 'message": "', '",');
